...
\begin{document}
    ...
    \newpage
    \section*{\begin{center}
        \underline{Pseudoc\'odigo}
    \end{center}}
    \begin{algorithm}
        \caption{Determinar el costo de la arista de menor costo entre las aristas de mayor costo en cada camino de $x$ a $y$ en un grafo $G$}
        \textbf{Solve($G,x,y$)\\}
        1-\hspace*{1em}DFS-Visit($G,x$) --------- Verificar si $y$ es alcanzable desde $x$. $O(V + E)$\\ 
        2-\hspace*{1em}if $\pi$[y] != NULL -------- Si $y$ es alcanzable desde $x$. $O(1)$\\
        3-\hspace*{2em}$T$ $\leftarrow$ PRIM($G,x$) --------- Construir un AACM de $G$ empezando en $x$. $O(E\log{V})$\\
        4-\hspace*{2em}DFS-Visit($T,x$) --------- Hallar el camino de $x$ a $y$ en $T$. $O(V + E)$\\
        5-\hspace*{2em}start $\leftarrow y$, answer $\leftarrow$ $w(\pi[y],y)$. $O(1)$\\ 
        6-\hspace*{2em}while start != x do:  $O(V)$\\
        7-\hspace*{3em}start $\leftarrow$ $\pi$[start] $O(1)$\\
        8-\hspace*{3em}answer $\leftarrow$ m\'ax(answer,w($\pi$[start],start)) $O(1)$\\
        9-\hspace*{2em} return answer\\\\

        l\'ineas 5-9 Quedarse con el mayor de los costos de las aristas en el camino de $x$ a $y$ en $T$
    \end{algorithm}

    \section*{\begin{center}
        \underline{Correctitud del Algoritmo}
    \end{center}}

    La correctitud del algoritmo se basa principalmente en la aplicaci\'on de la proposici\'on 1 previamente demostrada,
    una vez verificado que $y$ es alcanzable desde $x$, construido el AACM $T$ iniciando en $x$ y encontrado el camino de
    $x$ a $y$ en $T$, solo resta quedarse con la arista de mayor costo en dicho camino, operaci\'on que realizan las l\'ineas
    5-9 de pseudoc\'odigo usando el array $\pi$.

    \section*{\begin{center}
        \underline{Complejidad Temporal}
    \end{center}}

    La complejidad temporal del algoritmo es $O(V + E) + O(E\log{V}) + O(V + E) + O(V) = O(max(V + E, E\log{V}, V)) = O(E\log{V})$
\end{document}