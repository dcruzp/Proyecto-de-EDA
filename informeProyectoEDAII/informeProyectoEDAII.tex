...
\begin{document}
    ...
    se termine de ejecutar el algoritmo se cumplir\'a que $\exists e' \in E', e' \in E(T)$ donde $T$ es el AACM resultante al terminar el algoritmo de PRIM. Y como el resto de las aristas tiene costo
    menor o igual que dicha arista, el propio algortimo a\~nadir\'a el resto de las aristas en el camino\\\\
    ...
    Como $y$ es alcanzable desde $x$, uno de los caminos que van de $x$ a $y$ en $G$ tienen que estar en el AACM $T$ resultante, ahora una de las
    $e' \in E'$, de las cuales est\'an en $T$, tiene que estar en dicho camino, de lo contrario el algoritmo habr\'ia escogido un camino donde hay 
    aristas con costo mayor que la propia $e'$ lo que es imposible por las caracter\'isticas del algoritmo.\\\\
